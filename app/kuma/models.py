...
class Producto(models.Model):
    ...
    fecha_fin_garantía = models.DateField(null=True, blank=True)
    categoria_id = models.ForeignKey(Categoria, on_delete=models.CASCADE)
    imagen_url = models.ImageField(upload_to="ImagenesProductos/")

    def __str__(self):
        txt = "Nombre: {0} - ID: {1} - Descripcion: {2} - Stock: {3} - Precio: {4} - Fecha fin de garantía: {6}"
        return txt.format(self.nombre, self.sku, self.descripcion, self.stock, self.precio, self.fecha_fin_garantía)
